...
namespace SQMeeting.Utilities
{
    ...
    public class FRTCUIUtils
    {
        ...
        public static string AESDecrypt_ECB(byte[] cipherText, string strKey)
        {
            byte[] keyByteArray = Encoding.UTF8.GetBytes(strKey);
            string plaintext = null;
            using (RijndaelManaged rijAlg = new RijndaelManaged())
            {
                rijAlg.Key = keyByteArray;
            ...
            using (var searcher = new System.Management.ManagementObjectSearcher("Select * from Win32_ComputerSystem"))
            {
                using (var items = searcher.Get())
                {
                    foreach (var item in items)
                    {
                        ...
                        LogTool.LogHelper.Debug("Device Manufacturer: {0}", manufacturer);
                        LogTool.LogHelper.Debug("Device Model: {0}", item["Model"].ToString());
                        if ((manufacturer == "microsoft corporation" && item["Model"].ToString().ToUpperInvariant().Contains("VIRTUAL"))
                            || manufacturer.Contains("vmware")
                            || item["Model"].ToString() == "VirtualBox"
                            || manufacturer.ToLower() == "alibaba cloud")
                        {
                            return true;
                        }
                    }
