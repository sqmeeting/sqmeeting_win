...
namespace SQMeeting.Model
{
    ...
    public class FRTCUserManager
    {
        ...
        {
            FRTCUserManager self = GalaSoft.MvvmLight.Ioc.SimpleIoc.Default.GetInstance<FRTCUserManager>();
        ...
        public async void FRTCResetUserPassword(string OldPwd, string NewPwd)
        {
            ...
            LogTool.LogHelper.Debug(responseObj.StatusCode + Environment.NewLine + responseObj.Body);
        ...
        {
            LogTool.LogHelper.Debug("query users filter is " + filter);
            try
            {
                if (string.IsNullOrEmpty(_userToken))
                {
